...
const productsSlice = createSlice({
    ...
    reducers : {
        filterProducts(state,action){
            const {searchQuery, priceRange, categories : {mensFashion, womensFashion, jewelery, electronics}} = action.payload;
            ...
            if(mensFashion || womensFashion || jewelery || electronics){
                filteredProducts = filteredProducts.filter((product) => {
                    if(mensFashion && product.category === "men's clothing"){
                        console.log("mensFashion", product.category);
                        return true;
                    }
                    if(womensFashion && product.category === "women's clothing"){
                        console.log("womensFashion", product.category);
                        return true;
                    }
                    if(electronics && product.category === "electronics"){
                        console.log("electronics", product.category);
                        return true;
                    }
                    if(jewelery && product.category === "jewelery"){
                        console.log("jewelery", product.category);
                        return true;
                    }
                    return false;
                })
    ...
    extraReducers : (builder) => {
        ...
        .addCase(getAllProducts.fulfilled, (state, action) => {
            state.loading = false;
            const products = action.payload.docs.map((doc) => ({...doc.data()}))
            state.products = products;
            state.filteredProducts = products;
        })
        .addCase(getAllProducts.rejected,( state, action) => {
            state.loading = false;
            console.log("action getproducst", action.payload);
